...
<dialog data-dialog="{{ $name }}" aria-labelledby="{{ $name }}-title" @if($description) aria-describedby="{{ $name }}-description" @endif {{ $attributes->class('catalog-dialog m-auto w-[min(44rem,calc(100%-2rem))] max-h-[calc(100dvh-2rem)] rounded-panel border-0 bg-white p-0 text-ink shadow-2xl backdrop:bg-slate-950/40') }}>
