...
@if($cart->items->isNotEmpty())
    ...
    <div class="cart-request-list space-y-3">
        @foreach($supplierGroups as $supplierId => $group)
            ...
            <x-ui.dialog name="supplier-review-{{ $supplierId }}" class="cart-request-dialog">
                ...
                <div class="cart-dialog-footer"><form method="post" action="{{ route('cart.suppliers.checkout', $supplier) }}">@csrf <x-ui.button>Оформить заказ</x-ui.button></form><x-ui.button type="button" variant="secondary" data-dialog-open aria-controls="supplier-share-{{ $supplierId }}">Поделиться</x-ui.button><x-ui.button type="button" variant="secondary" data-dialog-open aria-controls="supplier-archive-{{ $supplierId }}">В архив</x-ui.button><x-ui.button type="button" variant="ghost" class="text-danger" data-dialog-open aria-controls="supplier-clear-{{ $supplierId }}">Очистить</x-ui.button></div>
            </x-ui.dialog>
    ...
    <x-ui.card class="cart-action-bar"><div><span class="text-sm text-muted">Итого к отправке</span><p class="text-xl font-bold text-brand-700" data-cart-total>{{ number_format((float) $cartTotal, 2, '.', ' ') }} TJS</p></div><div class="flex flex-wrap gap-2"><form method="post" action="{{ route('cart.checkout') }}">@csrf <x-ui.button>Оформить все</x-ui.button></form><x-ui.button type="button" variant="secondary" data-dialog-open aria-controls="cart-share-all">Поделиться</x-ui.button><x-ui.button type="button" variant="ghost" class="text-danger" data-dialog-open aria-controls="cart-clear-all">Очистить</x-ui.button></div></x-ui.card>
